...
+ [C]16kHz - 20kHz
+ [D]16kHz - 56kHz
> 钢琴弹奏出的C大调中央C音（middle C）为261.6Hz，再低上三、四个八度，频率就到十几赫兹了，为人耳的低音下限；中央C再高上五、六个八度，频率就到二十几k赫兹了，为人耳的高音上限。也就是说，人耳音频的上下限为十几赫兹到二十几k赫兹。  
> 这也是为什么48k采样率的数字音频，基本就比较高保真了（根据采样定理，采样频率要高于被采集样本频率范围的两倍）。
...
+ [C]17dBm
+ [D]35dBμ
> dB 本身是一个无量纲单位，其定义为两个数的比值取10的对数再乘10： dB = 10 * log10(P1 / P0)
> dBm 是某个功率相对于1mW功率的dB值，则： 1W = 1000mW = 10 * log10(1000mW / 1mW) = 10 * log10(1000) = 30dBm
> 关于log10的一些速记：log10(2) = 0.3, log10(5) = 0.7
> 5W = 10 * log10(5000mW / 1mW) = 10 * (log10(1000) + log10(5)) = 10 * (3 + 0.7) = 37dBm
...
+ [C]36dBm
+ [D]25dBm
> 同理，dBμ 是某个功率相对于1μW功率的dB值，则：
> 0.25W = 250000μW = 10 * log10(250000μW / 1μW) = 10 * (log10(10000) + log10(5)+ log10(5)) = 10 * (4 + 0.7 + 0.7) = 54dBμ
...
+ [C]6000dBm
+ [D]34dBm
> 0.4kW = 400000000μW = 10 * log10(400000000μW / 1μW) = 10 * (log10(100000000) + log10(2) + log10(2)) = 10 * (8 + 0.3 + 0.3)  = 86dBμ
...
+ [D]约50伏
> 有效电压 = 峰值电压 / 根号2
> 峰-峰指的是电压从正50v到负50v变动，总差值为100v，然后单峰值50除以根号2（1.414）为有效值35.4
...
+ [C]约141伏
+ [D]约50伏
> 注意这里峰值100v，100v/1.414=70.7v


## LK0518
峰-峰值为100伏的正弦交流电压，其 ***平均值*** 电压为：
+ [A] ***0***
+ [B]约35.4伏
+ [C]约70.7伏
+ [D]约141伏
...
+ [C]频率为F的奇数倍的无穷多个频率分量
+ [D]无穷多个连续的频率分量
> 涉及到傅立叶变换。正弦波其傅立叶变换就是其本身，就一个频率分量
...
+ [C]对称方波
+ [D]连续的无限窄脉冲
> 对于傅立叶变换，待分解的脉冲越窄，分解后的傅立叶谐波就越密，窄到无限窄后整个频谱内就都铺满频率分量了。这也就解释了为什么，一个电火花或者闪电（类似于单个窄脉冲）会对各个频段上的无线电接收机都会造成干扰）。
...
+ [C]自激振荡
+ [D]检波
> 检波是指将一个由信号调制的载波，检出其原来的信号部分；非线性失真一般是指输入信号与输出信号之间的波形不太一样了；自激振荡稍微复杂在此不做解释。滤波器会消减某些频率，肯定会引起频率分量的失真。


## LK0689
在给业余收发信机供电的整流电源中，开关电源可以做得比变压器直接降压整流的线性电源轻巧是因为：
+ [A]开关电源中变压器的工作频率高得多，可以 ***缩小磁性材料截面和减少线圈匝数***
...
+ [C]多条固定的直线
+ [D]一条复杂的周期性曲线
> 频谱仪上的曲线是按频率分布（横坐标为频率），FM频谱的中心固定线为基带频率，随着话音的变化其左右两边出现不同的频率分量线。
...
> 带宽BW=2 *（最大频偏+信号最高频率）。而频偏取决于信号幅度。其实际占用带宽跟调制的语音频率/幅度有关，不是固定值。
...
+ [A] ***衰减*** 是指信号通过信道或电路后 ***功率减少***， ***衰落*** 是指信号通过信道或电路后发生幅度 ***随时间而起伏***
...
## LK1044
无线电发信机在 ***无调制*** 情况下，在一个射频周期内供给天线馈线的平均功率称为：
+ [A] ***载波*** 功率
+ [B]无用功率
+ [C]平均功率
+ [D]峰包功率
...
## LK1099
调频发射机在发射的语音信号上附加一个 ***人耳听不到的低频音频，用来打开接收机的静噪***。这一技术的常用名词是：
+ [A] ***CTCSS***
+ [B]单音频脉冲
+ [C]DTMF
+ [D]载波静噪
...
> 注意FM的最大频偏仅取决于幅度，跟其基带信号频率没有关系，所以B不对。注意跟LK0786题的区别
...
## LK1202
以下哪种语音调制常被用于 ***长距离弱信号*** 的VHF或UHF联络？
+ [A] ***SSB***
+ [B]AM
+ [C]FM
+ [D]PM
> 问的是弱信号下。别选错成FM。注意跟LK1203题的区别
...
+ [C]天线和馈线上都只有驻波
+ [D]天线和馈线上都只有行波
> 驻波有利于向外进行电磁辐射。我们希望天线上尽量多的辐射，馈线上辐射尽量少。
...
> 水平方向没有指向性很好理解，因为其在水平方向360度全镜像对称，不可能有哪个方向信号更强一些。但其在垂直方向就不是全对称，因为竖向上能区分出上下两端和中部来。
...
> 理解GP天线首先要大致了解偶极天线。偶极天线有两条水平振子线，等长，每根都是1/4波长长度。
> GP天线实际上是把偶极天线竖着插到大地里，两条振子关于大地对称（就像一根在地里一根在外面）。由于大地可以当作电磁波的镜子，因此插入到地里的那根振子实际上就可以不要了（通过大地的镜子反射出一根虚拟的镜像振子来）。注意这里的大地，除开真实的地面，也可以是一大片金属当作大地，例如车顶。所以偶极天线两根1/4波长振子，而GP天线只需要一根1/4波长振子。
> 有的时候这个“大地”的反射情况不太理想，这时就须要增加一些水平铺在地上的地线帮助其完成镜像反射。


## LK0926
天线增益是指：
+ [A]天线在最大辐射方向上的辐射功率密度与相同条件下 ***基准天线*** 的辐射功率密度之比
+ [B]天线在接收或发射信号时所具有的放大倍数
+ [C]天线辐射到空间的电磁波功率与输入到天线的总射频功率之比
+ [D]输入到天线的总射频功率与天线辐射到空间的电磁波功率之比
> 增益都是个dB值，因此都是一个东西跟另一个东西的比较；某个天线的增益，对比的是基准天线。
> 有几种基准天线，如下面的点源天线，则增益单位为dBi；如下面的半波偶极天线，则增益单位为dBd。


## LK0929
某商品天线说明书给出的增益指标以 ***dBi*** 为单位。其意义为：
+ [A]“相对于无方向性 ***点源天线*** 的增益”，即最大辐射方向上的辐射功率密度与理想点源天线的辐射功率密度之比
...
## LK0930
某商品天线说明书给出的增益指标以 ***dBd*** 为单位。其意义为：
+ [A]“相对于半波长 ***偶极子天线*** 的增益”，即最大辐射方向上的辐射功率密度与半波长偶极振子的最大辐射功率密度之比
...
> 换算：有了某个天线的dBd值， 加上一个2.15就是它的dBi值。
...
+ [C]甲信号比乙信号强2.95dB
+ [D]乙信号比甲信号强7.1dB
> 2.9dBd=2.9+2.15=5.05dBi


## LK1062
以 ***dBd*** 为单位的天线增益则是指：
+ [A]最大辐射方向辐射功率密度与 ***半波长振子*** 最大辐射方向辐射功率密度之比的dB值
...
## LK1063
以 ***dBi*** 为单位的天线增益则是指：
+ [A]最大辐射方向辐射功率密度与 ***理想点源天线*** 最大辐射方向辐射功率密度之比的dB值
...
## LK1189
导致同轴电缆损害的 ***最常见*** 的原因是什么？
+ [A] ***电缆受潮***
+ [B]伽马射线
+ [C]速度因子超过了1
+ [D]过载
> B、D不太常见；C不对，同轴线缆速度因子都小于1。
...
## LK1211
下列关于 ***直立天线*** 的陈述，哪一项是正确的？
+ [A]该天线发射的电磁波 ***电场*** 垂直于地面
+ [B]该天线发射的电磁波磁场垂直于地面
+ [C]相位被反相了
+ [D]相位被倒转了
> 电场平面的切线就是天线导线，故而GP天线的电场平面垂直于地面。
...
+ [A]与 ***距离的平方成正比*** ，与 ***频率的平方成正比***
+ [B]与距离的平方成正比，与频率的平方成反比，
+ [C]与距离成正比，与频率的平方成正比
+ [D]与距离的平方成正比，与频率无关
> 注意两个都是平方关系。这里涉及到电磁波在自由空间传播损耗模型（弗里斯公式），比较复杂就不展开了。这是个理想模型，实际传播距离还要看绕射、反射、散射等多种情况，看天、看地、看天线方向性增益等。
...
+ [C]旧干电池的电动势为1.35伏
+ [D]旧干电池的电动势为0
> 电源电压=电源电动势-电源压降。电池用旧了是内部压降（内阻）升高而不是电动势降低。
...
+ [C]铅酸电池
+ [D]锂离子电池
> B是我们家里常用的1.5v充电电池；C是电瓶蓄电池；D是个大类，新能源充电电池很多都是某种锂离子电池，只不过阳极材料、电解质状态不同而已
...
+ [C]阿尔法辐射
+ [D]伽玛辐射
> 电离辐射是高能辐射，包括x射线、阿尔法α射线、贝塔射线β、伽马射线γ。通讯无线电波段能量低非电离，安全的多。
